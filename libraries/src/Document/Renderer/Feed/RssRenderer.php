...
class RssRenderer extends DocumentRenderer
{
	...
	public function render($name = '', $params = null, $content = null)
	{
		...
		for ($i = 0, $count = count($data->items); $i < $count; $i++)
		{
			...
			{
				if (in_array('matricules', $data->items[$i]->tags)) {
					$pathinfo = pathinfo($matches[1]);
					preg_match('/.*?([^_]*)$/', $pathinfo['filename'], $study);
					if (!empty($study) && file_exists($templatepath . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . 'twitter' . DIRECTORY_SEPARATOR . $study[1] . '-twitter-matricula.' . $pathinfo['extension'])) {
						$twitterimage = $templateurl . '/twitter/' . $study[1] . '-twitter-matricula.' . $pathinfo['extension'];
