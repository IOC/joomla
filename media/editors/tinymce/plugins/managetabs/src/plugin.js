...
tinymce.PluginManager.add('managetabs', function(ed, link) {
    ...
    var PLUGINVARS = {
        ...
        SELECTORS: {
            ...
            CLASS_STUDY_FRAME:     'study-frame',
            CLASS_STUDY_TERM:      'study-term'
        },
        ATTRIBUTES: {
            ID:                    'id',
            CLASS:                 'class',
        ...
        LANG: {
            ...
            CONTENT_SECTION:       '<p>Text de la secció</p>',
        }
    };
    ...
    ed.on('click', function(evt) {
        ...
        } else if (currentNode.tagName == PLUGINVARS.TAGS.DIV && currentNode.className.indexOf(PLUGINVARS.SELECTORS.CLASS_STUDY_TERM) > -1) {
            //Remove new section
            contentnode = currentNode.firstChild;
            if (confirm(PLUGINVARS.LANG.DELETE_SECTION + " '" + contentnode.textContent + "'?")) {
                currentNode.parentElement.removeChild(currentNode);
            }
        } else if (currentNode.tagName == PLUGINVARS.TAGS.DIV && currentNode.className.indexOf(PLUGINVARS.SELECTORS.CLASS_STUDY_FRAME) > -1) {
            //Add new section
            node = currentNode.getElementsByClassName(PLUGINVARS.SELECTORS.CLASS_STUDY_TERM)[0].cloneNode(true);
            contentnode = node.getElementsByTagName(PLUGINVARS.TAGS.H4)[0];
            contentnode.innerHTML = PLUGINVARS.LANG.NEW_SECTION;
            contentnode = node.getElementsByClassName(PLUGINVARS.SELECTORS.CLASS_STUDY_DEF)[0];
            contentnode.innerHTML = PLUGINVARS.LANG.CONTENT_SECTION;
            currentNode.appendChild(node);
        }
    });
});
